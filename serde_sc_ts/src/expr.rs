...
impl Field {
    pub fn new(ty: TypeExpr, optional: bool) -> Self {
        Self { ty, optional }
    }
}
...
impl TypeExpr {
    /// Returns a `TypeExpr` representing a `null` value.
...
impl TypeExpr {
    pub fn struct_insert(&mut self, k: String, v_type: TypeExpr) {
        ...
        s.record.insert(k, Field::new(v_type, false));
    }
